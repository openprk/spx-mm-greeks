...
const Heatmap: React.FC<HeatmapProps> = ({
  ...
  // Format exposure values consistently with other components
  // K = thousands of millions (billions)
  ...
  const plotData = useMemo(() => {
    ...
    if (expiration === 'ALL' && matrixData) {
      // Create 2D customdata array matching z matrix dimensions
      // customdata[i][j] corresponds to z[i][j] (expiration i, strike j)
      const customdata = matrixData.x_expirations.map((_expiration, _expIndex) =>
        matrixData.y_strikes.map((strike, _strikeIndex) => {
          // Try multiple key formats to handle decimal precision mismatch between backend (Python) and frontend (JS)
          const possibleKeys = [
            strike.toString(),           // "6620"
            strike.toFixed(1),          // "6620.0"
            Math.round(strike).toString(), // "6620" (rounded)
            strike.toString() + '.0',    // "6620.0"
          ];
          ...
          for (const key of possibleKeys) {
            if (matrixData.strike_details?.[key]) {
              details = matrixData.strike_details[key];
              foundKey = key;
              break;
            }
          }

          console.log(`🔍 Strike ${strike} - tried [${possibleKeys.join(', ')}], found: ${foundKey}, hasDetails: ${!!details}`);

          return details ? [
            details.regime_code || 'N/A',
            details.classification || 'No classification',
            formatExposure(details.gex || 0),
            formatExposure(details.dex || 0),
            formatExposure(details.vex || 0),
            formatExposure(details.cex || 0),
            details.call_oi || 0,
            details.put_oi || 0,
            details.pattern_flags?.length > 0 ? details.pattern_flags.join(', ') : ''
          ] : ['No data', 'No classification', '0', '0', '0', '0', 0, 0, ''];
        })
      );
      ...
      const heatmapTrace = {
        ...
        hovertemplate:
          ...
          `<b>GEX:</b> %{customdata[2]} | <b>DEX:</b> %{customdata[3]}<br>` +
          `<b>VEX:</b> %{customdata[4]} | <b>CEX:</b> %{customdata[5]}<br>` +
      ...
      ['GEX', 'DEX', 'VEX', 'CEX'].forEach(greek => {
        ...
        const customdata = strikes.map(strike => {
          ...
          return strikeData ? [
            ...
            formatExposure(strikeData.gex),
            formatExposure(strikeData.dex),
            formatExposure(strikeData.vex),
            formatExposure(strikeData.cex),
            strikeData.call_oi,
            strikeData.put_oi,
